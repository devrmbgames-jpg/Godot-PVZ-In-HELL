...
func query() -> QueryBuilder:
	return q.enabled().with_all([C_HazardFollow]).iterate([C_HazardFollow])


func process(entities: Array[Entity], components: Array, _delta: float) -> void:
	var follows: Array = components[0]
	for index: int in entities.size():
		var follow: C_HazardFollow = follows[index]
		...
		if not EntityAvailability.contains(follow.origin, _world):
			if follow.on_loss == DEF_Hazard.OwnerLoss.Despawn:
				...
				cmd.remove_component(entities[index], C_HazardFollow)
			continue
