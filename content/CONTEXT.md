...
Player carries `C_Strength` derived from `C_Attribute`; default `base=value=1.0`. Carry limits are global and mass-based for both raw and GECS rigid bodies: full-speed threshold `10 + 20 * Strength.value`, hard lift limit `90 + 30 * Strength.value`, with linear max-speed reduction between them. `C_CarryLoad` stores actual held mass only; `S_Motion` recomputes speed from current Strength and does not scale acceleration.

Generic Carry also accepts a scriptless/non-GECS `RigidBody3D` when it is within reach, unfrozen, not in group `no_carry`, and its mass is within the holder's Strength-derived limit. Targeting keeps this separately in `C_Interactor.physics_target`; pickup lazily creates a runtime Entity proxy with `C_PhysicsBodyRef` so the same C_HeldBy authority/lifecycle is preserved without modifying the original Node. Raw bodies are Carry-only with default `GrabControlProfile`; authored `C_Grabbable` overrides hand-slot/throw/rotation/hold behavior, never movement slowdown.
