...
`E_DaySession` supplies a fresh singleton C_PackageLedger beside C_DayCycle. PackageRegistrationService is the sole registration writer; ScanAction invokes it at the interaction command boundary. The held scanner reserves LMB, revalidates first-hit LOS and a 3 m range, and rejects Night/inactive actors/invalid parcels. All writes (ledger row, day sequence, runtime number, registered/scanned states) happen synchronously before feedback. IDs are ledger keys; numbers use `DDD-NNN`, with a separate sequence per registration cycle. Repeat scans return the original number across later cycles without a second row. Immutable shipment_number is not the runtime registration number.
