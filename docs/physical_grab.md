...
В C_Grabbable также доступны дистанция удержания, жёсткость и демпфирование, ограничения силы/момента и break_distance. Коэффициенты позиционной пружины описывают ускорение; solver учитывает массу и компенсирует гравитацию в пределах max_hold_force. Поворот использует кратчайшую quaternion-ошибку и тензор инерции. Это начальные настройки, пригодные для дальнейшего игрового тестирования.
...
Проверяются переходы владения, ограничения вместимости, отпускание/бросок, вращение и приоритет ввода, удаление участников, внешние изменения связей, модификаторы движения и чистая математика. Физические тесты действительно ждут шагов движка и проверяют удержание против гравитации, поворот моментом, препятствия и разрыв хвата. В тестах ввода заменяется только проверка захвата курсора: headless DisplayServer его не поддерживает.

Ручной игровой тест: поднимите по очереди три коробки, сравните разгон и бросок, повращайте предмет перед стеной, потяните его по полу и отпустите на ходу. Автоматические тесты не оценивают субъективное удобство управления и поведение реального геймпада.
