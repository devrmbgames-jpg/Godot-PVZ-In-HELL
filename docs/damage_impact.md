...
`O_HealthLifecycle` reacts only to `C_Living`, adds `C_Death` once and performs the existing control/grip cleanup. Package does not have C_Living and never receives C_Death. `O_PackageDamage` changes its condition to DAMAGED on the first applied damage and DESTROYED on depletion, releases interactions/cargo and clears ink. Destruction does not delete the physical package or release warehouse registration numbers.
